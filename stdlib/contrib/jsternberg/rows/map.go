...
func (t *mapTransformation) mapTable(in flux.Table, fn *execute.RowMapPreparedFn) (flux.Table, error) {
	...
	out, err := table.StreamWithContext(t.ctx, in.Key(), cols, func(ctx context.Context, w *table.StreamWriter) error {
		return in.Do(func(cr flux.ColReader) error {
			return t.mapValues(w, cr, fn)
		})
	})
	if err != nil {
		return nil, err
...
func (t *mapTransformation) mapValues(w *table.StreamWriter, cr flux.ColReader, fn *execute.RowMapPreparedFn) error {
	cols := w.Cols()
	...
	for i, n := 0, cr.Len(); i < n; i++ {
		...
		obj.Range(func(name string, v values.Value) {
			...
			if builders[idx] == nil {
				// Part of the group key. Verify
				// that we aren't trying to change it.
				j := execute.ColIdx(name, cr.Key().Cols())
				if !v.Equal(cr.Key().Value(j)) {
					err = errors.Newf(codes.FailedPrecondition, "map function attempted to modify the group key to a different value; column %q must be removed from the group key first", name)
				}
				return
			}

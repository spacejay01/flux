...
//			Tables       *function.TableObject `flux:"<-"`
...
//
// The `<-` string can be prepended or act alone to denote
// a pipe argument. Only one of these can be used for each
// spec definition and it does not apply for nested structs.
...
func readStructArg(fv reflect.Value, arg values.Value, name string, a *flux.Administration) error {
	...
	return ReadArgs(fv.Interface(), args, a)
}

// Argument defines the interface for reading an argument
// into a value.
type Argument interface {
	ReadArg(v values.Value, a *flux.Administration) error
}
...
func (t *TableObject) ReadArg(v values.Value, a *flux.Administration) error {
	o, ok := v.(*flux.TableObject)
	if !ok {
		return errors.Newf(codes.Invalid, "argument is not a table object: got %T", v)
	}
	t.TableObject = o
	a.AddParent(o)
	return nil
